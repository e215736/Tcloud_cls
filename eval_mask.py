"""
eval_mask.py とmean_std.pyがセット。
train_mask.py とmean_std_git.pyがセット。
...
#ファイルを空にする
acc_dir="/home/endolab/TN_classification_cloud/image/g-sam/acc_img"
miss_dir="/home/endolab/TN_classification_cloud/image/g-sam/miss_img"
...
def test_model(model, criterion):
    ...
    for index, (inputs, labels) in enumerate(dataloaders):
        ...
        with torch.no_grad():
            ...
            if preds==labels:
                ...
                shutil.copy(dataloaders.dataset.samples[index][0], "./image/g-sam/acc_img/")
            # 誤画像の保存
            else:
                print(dataloaders.dataset.samples[index][0])                
                shutil.copy(dataloaders.dataset.samples[index][0], "./image/g-sam/miss_img")    
    ...
    plt.savefig('./image/g-sam/conf_matrix/'+date+'_G-SAMconfusion_matrix.png')
...
G_acc_dir="/home/endolab/TN_classification_cloud/image/g-sam/gradcam/acc"
G_miss_dir="/home/endolab/TN_classification_cloud/image/g-sam/gradcam/miss"
...
for trg in target:
    # 読み込みディレクトリ
    input_dir = f"./image/g-sam/{trg}_img" 

    # 出力ディレクトリ
    output_dir = f"./image/g-sam/gradcam/{trg}"
