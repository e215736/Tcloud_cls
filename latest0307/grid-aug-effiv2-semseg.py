...
from torchvision import datasets, transforms, models # modelsを追加
...
from mean_std_semseg import calculate_mean_std
...
class EfficientNetV2_B0(nn.Module):
    def __init__(self, class_num):
        ...
        # classifierの置き換え (最終層の出力次元数に注意)
        self.base_model.classifier = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(1280, class_num)  # efficientnetv2_b0の出力次元に合わせて調整
        )
...
data_folder_name = 'train_semseg'
test_folder_name = 'test_semseg'
...
def test_model(model, criterion, dataloaders, device, phase="test"):
    ...
    plt.title(f'efficientnet vanilla Confusion Matrix ({phase}) Fold{fold+1}')
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.xticks(rotation=45)
    plt.yticks(rotation=0)

    plt.savefig(f"./image/aug-semseg0.0001/conf_matrix/{phase}_confusion_matrix_fold{fold + 1}.png")
    plt.close()
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' : #and epoch_acc > best_acc:  変更点
                if epoch_loss < best_loss: # lossが減少したとき
                    ...
                    # 各foldの最良モデルを保存
                    model_save_path = f"./model/CCSN_3class-aug-semseg0.0001/aug-best_eff_model_fold{fold}.pth" #保存パス
...
for fold, (train_index, val_index) in enumerate(kf.split(train_val_dataset , targets)):
    ...
    # # モデルの定義 (EfficientNetB0を使用)
    # model = EfficientNetB0(class_num=len(class_names)) # モデルの変更点
    # model.to(device)
    # モデルの定義 (EfficientNetV2-B0を使用)
    ...
    plt.savefig(f'./image/aug-semseg0.0001/learning-curve/loss_fold{fold+1}.png')
    plt.close()
    ...
    plt.savefig(f'./image/aug-semseg0.0001/learning-curve/accuracy_fold{fold+1}.png')
    plt.close()
...
# 結果出力部分をファイルに書き込む
res_file_path = "/home/endolab/TN_classification_cloud/image/aug-semseg0.0001/res.txt"  # ファイルパス
