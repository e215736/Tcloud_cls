...
#model_names = ['vit_large_patch16_224.augreg_in21k'] #修正
model_names = ['vit_small_patch16_224.augreg_in21k']
# model_names = ['vit_base_patch16_224_in21k'] #修正
...
def test_model(model, criterion, dataloaders, device, phase="test"):
    ...
    plt.title(f'semseg-ViT Confusion Matrix ({phase}) Fold{fold+1}')
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' : #and epoch_acc > best_acc:  変更点
                if epoch_loss < best_loss: # lossが減少したとき
                    ...
                    # 各foldの最良モデルを保存
                    model_save_path = f"./model/CCSN_3class-vit-semseg/gridsearch/best_{model_name}_batch{batch_size}_{optimizer_name}_lr{lr}_fold{fold}.pth"
...
for batch_size in batch_sizes:
    for model_name in model_names:
        for optimizer_name in optimizers:
            ...
            # 結果を保存するディレクトリを作成
                result_dir = f"./image/aug-semseg-vit-gridsearch/{model_name}_batch{batch_size}_{optimizer_name}_{lr}"
