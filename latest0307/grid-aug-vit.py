        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' : #and epoch_acc > best_acc:  変更点
                if epoch_loss < best_loss: # lossが減少したとき
                    ...
                    # 各foldの最良モデルを保存
                    model_save_path = f"./model/CCSN_3class-vit/gridsearch/best_{model_name}_batch{batch_size}_fold{fold}.pth"  # モデルの保存パス
...
for batch_size in batch_sizes:
    for model_name in model_names:
        for optimizer_name in optimizers:
            # 結果を保存するディレクトリを作成
            result_dir = f"./image/aug-vit-gridsearch/{model_name}_batch{batch_size}_{optimizer_name}"
