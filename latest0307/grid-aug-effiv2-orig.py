...
def test_model(model, criterion, dataloaders, device, phase="test"):
    ...
    plt.savefig(f"./image/testaug-orig0.0001/conf_matrix/{phase}_confusion_matrix_fold{fold + 1}.png")
    plt.close()
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' : #and epoch_acc > best_acc:  変更点
                if epoch_loss < best_loss: # lossが減少したとき
                    ...
                    # 各foldの最良モデルを保存
                    model_save_path = f"./model/CCSN_3class-testaug-orig0.0001/aug-best_eff_model_fold{fold}.pth" #保存パス
...
for fold, (train_index, val_index) in enumerate(kf.split(train_val_dataset , targets)):
    ...
    plt.savefig(f'./image/testaug-orig0.0001/learning-curve/loss_fold{fold+1}.png')
    plt.close()
    ...
    plt.savefig(f'./image/testaug-orig0.0001/learning-curve/accuracy_fold{fold+1}.png')
    plt.close()
...
# 結果出力部分をファイルに書き込む
res_file_path = "/home/endolab/TN_classification_cloud/image/testaug-orig0.0001/res.txt"  # ファイルパス
